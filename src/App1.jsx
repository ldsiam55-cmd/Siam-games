...
export default function App() {
  ...
  const resetQuiz = () => {
    setDifficulty(null); setQuizPhase("select");
  };
      ...
      <header style={s.header}>
        <div style={s.headerTop}>
          ...
          <div style={s.scorePill}>
            ...
            <div style={s.scoreNum}>{totalScore.toLocaleString()}</div>
          </div>
        </div>
        <nav style={s.nav}>
          {[{id:"home",label:"🏠 Home"},{id:"guess",label:"🎯 Guess Player"},{id:"teams",label:"🏆 Top Teams"},{id:"rankings",label:"📊 Rankings"},{id:"leaderboard",label:"🎖️ Leaderboard"}].map(n => (
            <button key={n.id} style={{...s.navBtn,...(page===n.id?s.navActive:{})}} onClick={()=>goTo(n.id)}>{n.label}</button>
          ))}
        </nav>
      </header>
          ...
          <div>
            ...
            <div style={s.grid4}>
              {[
                ...
                <div key={c.page} style={{...s.card,borderTop:`5px solid ${c.accent}`}} onClick={()=>goTo(c.page)}>
          ...
          <>
              ...
              <div>
                ...
                <div style={s.diffGrid}>
                  {Object.entries(DIFF).map(([key,cfg]) => (
                    <div key={key} style={{...s.diffCard,borderTop:`5px solid ${cfg.color}`}} onClick={()=>startQuiz(key)}>
              ...
              <div>
                <div style={s.quizHeader}>
                  ...
                  <div style={s.quizScoreBox}>
                    ...
                    <div style={{fontSize:30,fontWeight:900,color:d.color}}>{roundScore}</div>
                  </div>
                </div>
                <div style={s.progressBar}>
                  <div style={{...s.progressFill,width:`${(currentIdx/players[difficulty].length)*100}%`,background:d.color}} />
                </div>
                <div style={s.quizCard}>
                  ...
                  <div style={s.hintsArea}>
                    {cp.hints.slice(0,hintIdx+1).map((h,i) => (
                      <div key={i} style={{...s.hintItem,borderColor:i===hintIdx?d.color:"rgba(0,100,255,0.18)"}}>
                  ...
                  {!answered && hintIdx < cp.hints.length-1 && (
                    <button style={{...s.hintBtn,borderColor:d.color,color:d.color}} onClick={()=>setHintIdx(i=>i+1)}>
                  ...
                  {!answered ? (
                    <div style={s.inputRow}>
                      ...
                      <button style={{...s.guessBtn,background:d.color}} onClick={handleGuess}>GUESS ⚽</button>
                    </div>
                  ) : correct ? (
                    <div style={{textAlign:"center"}}>
                      <div style={{...s.resultBanner,background:"#072a12",border:"2px solid #00cc44"}}>
                        <div style={{fontSize:52}}>✅</div>
                        <div style={{fontSize:30,fontWeight:900,color:"#00ff66"}}>CORRECT! +{POINTS[hintIdx]} pts</div>
            ...
            {quizPhase === "gameover" && cp && (
              <div style={{textAlign:"center",padding:"40px 20px"}}>
                ...
                <h2 style={{fontSize:44,fontWeight:900,color:"#ff4444",margin:"16px 0"}}>GAME OVER!</h2>
                ...
                <div style={{display:"flex",gap:16,justifyContent:"center",marginTop:30,flexWrap:"wrap"}}>
                  <button style={s.retryBtn} onClick={()=>startQuiz(difficulty)}>🔄 Try Again</button>
                  <button style={s.changeDiffBtn} onClick={resetQuiz}>🎯 Change Difficulty</button>
                </div>
              </div>
            )}
            ...
            {quizPhase === "complete" && (
              <div style={{textAlign:"center",padding:"40px 20px"}}>
                <div style={{fontSize:80}}>🏆</div>
                <h2 style={{fontSize:44,fontWeight:900,color:"#ffd700",margin:"16px 0"}}>COMPLETED!</h2>
                <p style={{fontSize:22,color:"#778"}}>You finished {d.label} mode without a single mistake!</p>
                <div style={s.resultBox}>
                  <div style={{fontSize:20,color:"#aaa"}}>Final Score</div>
                  <div style={{fontSize:60,fontWeight:900,color:d.color,margin:"10px 0"}}>{roundScore}</div>
                  <div style={{fontSize:18,color:"#778"}}>pts from {players[difficulty].length} players</div>
                </div>
                <div style={{display:"flex",gap:16,justifyContent:"center",marginTop:30,flexWrap:"wrap"}}>
                  <button style={s.retryBtn} onClick={()=>startQuiz(difficulty)}>🔄 Play Again</button>
                  <button style={s.changeDiffBtn} onClick={resetQuiz}>🎯 Change Difficulty</button>
                </div>
              </div>
            )}
        ...
        {page === "teams" && (
          <div>
            ...
            <div style={s.teamGrid}>
              {topTeams.map(t => (
                <div key={t.name} style={{...s.teamCard,borderTop:`4px solid ${t.accent}`,boxShadow:`0 0 20px ${t.accent}22`}}>
        ...
        {page === "leaderboard" && (
          <div>
            ...
            <div style={s.yourScoreBox}>
              Your Total Score: <strong style={{color:"#0099ff",fontSize:30}}>{totalScore.toLocaleString()}</strong> pts
            </div>
...
const s = {
  ...
  progressFill:{height:"100%",borderRadius:6,transition:"width 0.4s"},
  ...
  input:{flex:1,background:"rgba(255,255,255,0.06)",border:"2px solid rgba(255,255,255,0.12)",borderRadius:14,padding:"16px 20px",color:"#fff",fontSize:18,outline:"none"},
  ...
  resultBox:{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:"32px 24px",margin:"28px auto",maxWidth:480},
